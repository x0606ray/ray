...
　 .then(function(response){
    ...
    // console.log(response.data.forecasts[0].telop);
 let mes=''
    mes = `${response.data.forecasts[0].date}${response.data.location.city}の天気は${response.data.forecasts[0].telop}、最高気温は${response.data.forecasts[0].temperature.max.celsius}度だよ？まだまだ夏だな？`
    console.log(mes)
